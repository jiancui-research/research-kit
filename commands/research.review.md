...
After automatic setup, it is **report-only**: it writes mock reviews + scores to `review/round-N.md` and nothing else. It does not edit the paper and does not write into any other lane (no auto-appending to `tasks/`). Each finding carries a **suggested fix command** so you can route it yourself, then re-run `/research.review` for the next round.

Implementation and writing check internal consistency as part of their work. This command evaluates only what the submitted paper tells an outside reader. Do not load the internal consistency guide or project evidence during a review.
...
1. **Read the paper only.**
   ...
   - Read the manuscript — resolve its root from `./.research/paper-repo` (use an explicit path first, then the pointer, then the current manuscript repo, then `./paper/`). This is the *only* artifact you review. If a section is missing or still an outline, treat it exactly as a reviewer would: a gap in the submission.
   ...
   - A guide that is not in this repo is a gap to name, not to fill from memory: let setup fill missing templates, name any guide still unavailable, and report the reduced review coverage. Never reconstruct it from memory.
...
5. **Suggest a fix command per finding (do not run it or write to its file).** For each finding, name the one command the *user* would run to fix it — a suggestion in the report, not an action this command takes:
   - unsupported / overclaimed text, framing, voice -> `/research.write <section>`
   ...
   - a number that looks wrong or internally inconsistent → `/research.implement check` (trace it across artifacts in a separate run)
...
Write `./.research/review/round-N.md` (start from `.research/templates/review-template.md` if it exists), creating `./.research/review/` as needed and **never overwriting an existing round file**. After automatic setup, this is the **only** file this command writes. Structure it as the panel output, not prose:
...
- After setup, only `review/round-N.md` was written; paper, claims, evals, and tasks were untouched.

## Completion

Report `./.research/review/round-N.md`, the panel score range, and finding counts. Then: resolve findings via their suggested commands (`/research.write <section>`, `/research.relatedwork`, `/research.implement`, ...), and re-run `/research.review` until no high-severity findings remain.
