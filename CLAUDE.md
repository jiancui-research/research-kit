...
2. **Plugin marketplace** (`.claude-plugin/marketplace.json` + `plugin.json`) - zero-script path for Claude Code and OMP, both reading `commands/` directly. Stages get namespaced as `/research-kit:research.<name>`. **Codex and Copilot must use the script** (re-verified 2026-07): Codex installs the bundle but converts commands to skills and drops every one using `$ARGUMENTS` (1 of 14 survives), and Copilot now wants root `plugin.json` + `agents/`/`skills/` and surfaces nothing from `commands/`. See `docs/design.md` for the details and what a skills-based unification would cost.
