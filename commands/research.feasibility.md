...
3. **Design the minimal probe, paper-type aware.** Pull the relevant dimensions from `.research/templates/paper/<type>.md` if present, then pick the matching probe:
   - **attack / technique** — a **minimal validation experiment**: the smallest end-to-end run that shows the core mechanism *works at all* (e.g. one exploit fires on one target, one method beats trivial baseline on a toy input). Does the idea function outside the slides?
