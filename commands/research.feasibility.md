...
It is a self-contained **plan → do → decide**: a short *Probe plan* (a few throwaway tasks), the finding, and the verdict — all in `feasibility.md`. Those probe tasks are deliberately separate from the rigorous `tasks/eval.md`: they inform GO/NO-GO, not the paper's claims.
