...
- `/research.style` — Optional: distill the exemplars in `.research/style/samples/` into `.research/style/voice.md`, a living register profile the writing commands load. Patterns and slots only, never sentences copied from a sample. Voice precedence when writing: the manuscript's own prose > the voice profile > the craft guides.
...
- `/research.mdsplit` — The same server (`--split`) in the original source-beside-preview layout, for when seeing raw and rendered at once matters. Shares the `.mdreview/` comments; a running instance of one mode is never reused for the other.
...
<project>/                 one repo under ~/Projects, outside the vault
  .research/               ALL research-kit tracking docs:
    ...
    style/                   optional: samples/ + voice.md (from /research.style)
...
**Why Codex and Copilot use the script instead** (re-verified 2026-07, Codex CLI 0.145.0 / Copilot CLI 1.0.63). Both now accept the bundle, and both then drop the pipeline. Codex resolves `.claude-plugin/marketplace.json` as a legacy path and installs cleanly, but converts commands into skills and silently skips every command containing `$ARGUMENTS` — 1 of 14 stages survives (`research.init`, the only argument-free one). Copilot's plugin format expects `plugin.json` at the repo root plus an `agents/` or `skills/` directory and no longer surfaces `commands/`; an installed bundle reports zero available agents, though it worked at Copilot CLI 1.0.40 when this was first verified. Unifying on a single marketplace install would mean authoring stages as `skills/<name>/SKILL.md`, which every one of these agents reads — at the cost of making the stages model-invocable rather than strictly user-driven.
