...
**Why Codex and Copilot use the script instead** (re-verified 2026-08, Codex CLI 0.149.1 / Copilot CLI 1.0.74). Both accept the bundle, report it installed, and then deliver nothing. Codex resolves `.claude-plugin/marketplace.json` as a legacy path and installs cleanly — `codex plugin list` prints `installed, enabled` — but surfaces **0 of 16** stages; `research.init`, the one argument-free stage that survived at 0.145.0, no longer appears either. Codex reads a plugin's `skills/` directory and this bundle carries `commands/`. Its documented format is now `.codex-plugin/plugin.json` + `skills/<name>/SKILL.md` ([plugin docs](https://learn.chatgpt.com/docs/build-plugins)), with SKILL.md frontmatter limited to `name` and `description` — no argument substitution at all, so shipping that layout would strand 15 of 16 stages regardless. Custom prompts (`~/.codex/prompts/`) remain the only Codex surface that takes input: they support `$ARGUMENTS`, `$1`–`$9`, and named `KEY=value` placeholders, and are deprecated in favour of skills with no announced removal date. Copilot's plugin format expects `plugin.json` at the repo root plus an `agents/` or `skills/` directory and no longer surfaces `commands/`; an installed bundle reports zero available agents, though it worked at Copilot CLI 1.0.40 when this was first verified. Unifying on a single marketplace install would mean authoring stages as `skills/<name>/SKILL.md`, which every one of these agents reads — at the cost of making the stages model-invocable rather than strictly user-driven.
