...
- **Self-pruning.** Re-running `install.sh` removes any installed `research.*` commands that were deleted from the bundle, so upgrading self-cleans — no stale commands linger in your `/` menu.
