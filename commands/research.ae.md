...
## Steps
1. Read `./.research/memory/constitution.md` if present. Read upstream artifacts when available: `./.research/claims.md` (which results need to be reproducible), `./.research/tasks/experiment.md` (datasets, baselines, hardware, hyperparameters), and `./.research/experiments/` (concrete commands and configs).
...
## Completion
Report the paths written under `./.research/ae/`. This is an auxiliary command, not a pipeline stage: it runs in parallel once experiment results exist, and has no next step.
