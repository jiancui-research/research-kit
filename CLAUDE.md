...
2. **Plugin marketplace** (`.claude-plugin/marketplace.json` + `plugin.json`) - zero-script path for Claude Code and OMP, both reading `commands/` directly. Stages get namespaced as `/research-kit:research.<name>`. **Codex reads a plugin's `skills/`, so the bundle now ships one** (`.codex-plugin/plugin.json` + `skills/<name>/SKILL.md`, generated from `commands/` by `tools/gen_codex_skills.py` - each SKILL.md is a pointer at its command, never a copy, and `tools/test_codex_skills.py` fails if the committed tree drifts). Through 0.37.1 Codex reported the bundle installed and surfaced 0 of 16 stages. **Copilot must still use the script** (re-verified 2026-08, 1.0.74): it wants root `plugin.json` + `agents/`/`skills/` and surfaces nothing from `commands/`. See `docs/design.md` for the details and what a skills-based unification would cost.
