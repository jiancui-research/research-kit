...
2. **Plugin marketplace** (`.claude-plugin/marketplace.json` + `plugin.json`) - zero-script path for Claude Code and OMP, both reading `commands/` directly. Stages get namespaced as `/research-kit:research.<name>`. **Codex and Copilot must use the script** (re-verified 2026-08, Codex 0.149.1 / Copilot 1.0.74): both report the bundle installed and expose nothing from `commands/` - Codex now surfaces 0 of 16 stages (even argument-free `research.init`, which survived at 0.145.0), because it reads a plugin's `skills/` and its own format is `.codex-plugin/plugin.json` + `skills/<name>/SKILL.md` whose frontmatter is `name`+`description` only. See `docs/design.md` for the details and what a skills-based unification would cost.
