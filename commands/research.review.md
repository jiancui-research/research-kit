---
description: Self-review panel + router. Simulate a reviewer panel, score the draft, route each finding to its owning command, auto-append evidence-gap evals, and loop until clean. Writes .research/review/round-N.md; never rewrites the paper.
...
This is the **self-review loop**: review your OWN draft the way a skeptical program-committee panel will, **before** submission. This command is a **router and tracker**, not an editor. It simulates a panel of reviewers, writes mock reviews + scores to `review/round-N.md`, **routes** each finding to the command that owns the fix, and **auto-appends** new eval tasks for evidence gaps to `tasks/eval.md`. It **never rewrites the paper** and never edits a claim, eval, or section to make a finding go away — it records the finding and hands it off. (Distinct from `/research.analyze`, which is an objective read-only consistency audit; review is subjective panel judgment + routing + a scored loop.)

## Steps

1. **Read the artifacts (read-only on all but the round file and the eval task list).**
   - Read `./.research/memory/constitution.md` if present (venue, paper-type, voice); skip silently if absent.
   - Read what exists: `./.research/proposal.md`, `./.research/related-work.md`, `./.research/feasibility.md`, `./.research/tasks/eval.md`, `./.research/tasks/paper.md`, `./.research/claims.md`, `./eval/`, and `./paper/`. Note any missing artifact as a gap rather than failing.
...
2. **Convene the panel.** Simulate **3–4 reviewers, each with a distinct lens**, so findings do not collapse into one voice:
   ...
   - **R2 — Empiricist / evaluation:** do the evals directly test each claim? fair tuned baselines, reported variance, no leakage, validated automatic/LLM judges? (cross-check `claims.md` and `eval/`).
...
5. **Route every finding to its owning command.** This is the load-bearing step — each finding gets exactly one next action:
   ...
   - a claim that needs new or stronger evidence → an eval (see step 6) and `/research.eval` to run it
   - a result that exists but is mis-stated in `claims.md` → `/research.eval` (re-score the claim)
   - a feasibility / GO-NO-GO concern or a contribution gap → `/research.feasibility` or `/research.proposal`

6. **Auto-append eval tasks for evidence gaps.** For each finding that needs new evidence, **append** a task to `./.research/tasks/eval.md` (do not rewrite existing tasks): one line per gap, tagged `from review round N`, naming the claim id it backs and the verdict it must produce. This keeps the eval/paper loop fed from the review.
...
- **Auto-appended evals**: the list of tasks added to `tasks/eval.md` this round (claim id + required verdict + `from review round N`).
...
- Every evidence gap was appended to `tasks/eval.md`, tagged `from review round N`.
- Scores trace to the findings, not to a gut feeling; desk-reject gate checked.
- No paper section, claim, or eval was rewritten by this command.

## Completion

Report the path `./.research/review/round-N.md`, the panel score range, and the counts (fatal / major / minor findings, and evals appended). Then end with: `Next: resolve findings via their routed commands (/research.paper, /research.relatedwork, /research.eval, ...), then re-run /research.review for round N+1; loop until no high-severity findings remain. (/research.rebuttal is auxiliary — post-submission.)`
