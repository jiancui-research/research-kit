...
- **Pipeline order.** `constitution -> proposal -> relatedwork -> feasibility -> tasks -> experiment + paper (in parallel) -> analyze -> review (loop)`, plus `rebuttal` and `ae` as needed. Don't reorder it casually.
