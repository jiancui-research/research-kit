...
- 💬 **Comment on either side, fix in the source.** Select text in the rendered paper *or* in the LaTeX, then right-click it to attach a note (selecting alone no longer interrupts you). Source comments know their `file:line` outright and open the LaTeX when you click them back. Each comment records the quote *and* the SyncTeX-resolved `file.tex:line`, so an agent working in the paper repo knows exactly where to edit — *"read `.texreview/comments.json` and address the open comments"*. It replies with what it changed and marks them resolved; the fix is highlighted on the next compile.
