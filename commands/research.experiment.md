...
3. Decide the action from $ARGUMENTS:
   - **Scaffold** — for each experiment implied by the plan (or named by the user), create `./.research/experiments/NN-slug.md` from `templates/experiment-template.md`. Number sequentially (`01`, `02`, …); `slug` is a short kebab-case name. Never overwrite an existing file without saying so.
