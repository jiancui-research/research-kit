...
- **Working directory.** Commands read and write only under `./.research/` in the user's own paper repo, with one deliberate exception: the `design` lane builds **code** in its own repo under `~/Projects/<repo>` (outside the vault). They `mkdir -p` as needed and never overwrite user content without saying so.
- **Pipeline order.** `constitution -> proposal -> relatedwork -> feasibility -> tasks -> (design ∥ experiment ∥ paper, in parallel) -> analyze -> review (loop)`, plus `rebuttal` and `ae` as needed. After `tasks` the three lanes co-evolve and are kept in sync by `analyze`; the `design` lane is paper-type aware (skipped for measurement / SoK). Don't reorder it casually.
